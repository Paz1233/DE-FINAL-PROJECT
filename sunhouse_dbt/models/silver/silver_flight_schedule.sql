{{ config(
    ...
    schema='sunhouse',
    properties={
        'location': "'abfs://warehouse@dataengineering2025sa.dfs.core.windows.net/SunHouse/Silver/flight-schedule/'"
    }
) }}
...
WITH raw_extracted AS (
    SELECT
        ...
        destination,
        airline
    FROM {{ source('bronze', 'raw_flight_schedule') }}
...
surrogate_key_generation AS (
    SELECT
        ...
        {{ generate_flight_id('clean_flight_number', 'clean_departure_date') }} AS flight_id,
